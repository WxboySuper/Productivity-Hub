...
describe("useTasks", () => {
  ...
  describe("CSRF token helper", () => {
    it("should handle empty CSRF token response", async () => {
      ...
      expect(
        mockFetch.mock.calls.some((call) => call[0] === "/api/csrf-token"),
      ).toBe(true);

      // Check that /api/tasks/1 was called with X-CSRFToken: ''
      const taskCall = mockFetch.mock.calls.find(
        (call) => call[0] === "/api/tasks/1",
      );
  ...
  describe("state management", () => {
    ...
    it("should maintain loading state properly", async () => {
      ...
      mockFetch.mockReturnValueOnce(
        delayedPromise as unknown as Promise<Response>,
      );
