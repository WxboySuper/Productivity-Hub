...
const MainManagementWindowWrapper = ({
  ...
  <BrowserRouter>
    <AuthProvider>
      <BackgroundProvider>
        <ToastProvider>{children || <MainManagementWindow />}</ToastProvider>
      </BackgroundProvider>
    </AuthProvider>
  </BrowserRouter>
);
...
describe("MainManagementWindow - Additional Coverage", () => {
      ...
      if (projectTasks.length > 0) {
        ...
        if (editButtons.length > 0) {
          ...
          await waitFor(() => {
            ...
            const formContainer = screen.queryByPlaceholderText(
              "What needs to be done?",
            );
            if (formContainer) {
  ...
  describe("CSRF Token Edge Cases", () => {
    ...
    it("handles existing CSRF token in cookie", async () => {
      ...
      expect(global.fetch).not.toHaveBeenCalledWith("/api/csrf-token", {
        credentials: "include",
      });
