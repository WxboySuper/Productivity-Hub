...
describe("Task Form", () => {
  ...
  it("handles task completion toggling", async () => {
    ...
    fetchMock.mockImplementation((url, options) => {
      if (url === "/api/projects") {
        return Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve({ projects: [{ id: 1, name: "Test Project" }] }),
        } as Response);
      }
      if (
        url === "/api/tasks" &&
    ...
    const projectsSidebarBtn = screen
      .getAllByText("Projects")
      .find((el) => el.closest("button"))
      ?.closest("button");
    if (!projectsSidebarBtn)
      throw new Error("Projects sidebar button not found");
      ...
      await waitFor(
        () => {
          const taskTitles = Array.from(
            document.querySelectorAll(".phub-item-title"),
          );
          found = taskTitles.some(
            (el) => el.textContent?.replace(/\s+/g, " ").trim() === "Test Task",
          );
  ...
  it("handles task deletion", async () => {
    ...
    const taskCard = taskCards.find((card) =>
      card.textContent?.includes("Test Task"),
    );
    expect(taskCard).toBeTruthy();

    const taskDeleteButton = Array.from(
  ...
  it("opens task details when clicking on task title", async () => {
    ...
    await waitFor(
      () => {
        ...
        const taskTitleNode = taskTitles.find(
          (el) => el.textContent?.replace(/\s+/g, " ").trim() === "Test Task",
        );
  ...
  it("handles task form error display", async () => {
    ...
    await waitFor(() => {
      const errorDiv = Array.from(
        document.querySelectorAll(".modern-error"),
      ).find((el) => el.textContent?.includes("Task creation failed"));
      if (errorDiv) {
