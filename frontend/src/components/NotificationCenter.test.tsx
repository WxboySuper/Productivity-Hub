...
describe('NotificationCenter', () => {
  ...
  beforeEach(() => {
    ...
    mockNotificationStatic.requestPermission = vi.fn().mockResolvedValue('granted');
    mockNotificationConstructor.mockImplementation((title: string, options?: NotificationOptions) => {
  ...
  describe('Notification Display', () => {
    ...
    it('should display notifications with correct styling based on read status', async () => {
      ...
      const unreadNotification = screen.getByText('Complete project proposal').closest('.border-blue-200');
      expect(unreadNotification).toHaveClass('border-blue-200', 'bg-gradient-to-r');
      
      // Check read notification styling
      const readNotification = screen.getByText('Review meeting notes').closest('.border-gray-200');
  ...
  describe('Notification Actions', () => {
    it('should dismiss notification when dismiss button is clicked', async () => {
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith('/api/notifications/1/dismiss', {
          method: 'POST',
          headers: {
    ...
    it('should snooze notification when snooze button is clicked', async () => {
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith('/api/notifications/1/snooze', {
          method: 'POST',
          headers: {
    ...
    it('should not show action buttons for read notifications', async () => {
      ...
      const readNotificationContainer = screen.getByText('Review meeting notes').closest('div');
  ...
  describe('Modal Notifications', () => {
    ...
    it('should create browser notification when permission is granted', async () => {
      ...
      expect(mockNotificationConstructor).toHaveBeenCalledWith('Task Reminder', {
    ...
    it('should dismiss modal when close button is clicked', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('/api/notifications/4/dismiss', {
        method: 'POST',
        headers: {
          'X-CSRF-Token': 'test-token',
        },
  ...
  describe('Browser Notification Permissions', () => {
    it('should request notification permission on mount', async () => {
      ...
      Object.defineProperty(window, 'Notification', {
        value: Object.assign(mockNotificationConstructor, mockNotificationStatic),
        configurable: true,
      });
