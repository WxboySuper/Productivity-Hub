...
const NotificationCenter: React.FC<NotificationCenterProps> = ({
    ...
    const fetchNotifications = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const nextToShow = data.find(
            (n: Notification & { show_at?: string }) => {
          ...
          if (nextToShow) {
            if (Notification.permission === "granted") {
              ...
              const _ = new Notification("Task Reminder", {
                body: nextToShow.message,
              });
            }
  ...
  const handleNotificationDismiss = (id: number) =>
    handleDismiss.bind(null, id);
  const handleNotificationSnooze = (id: number) =>
    handleSnooze.bind(null, id, 10);
      ...
      {modalNotification && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-lg shadow-2xl max-w-md w-full p-6 relative flex flex-col items-center border-2 border-yellow-400">
            ...
            <p className="mb-4 text-gray-800 text-lg text-center">
              ...
              {notifications.map((n) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <span
                      className={`${n.read ? "text-gray-600" : "text-blue-900 font-medium"} flex-1 mr-3`}
                    >
                      {n.message}
                    </span>
                    {!n.read && (
