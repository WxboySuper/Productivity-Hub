...
import { AuthProvider, useAuth, getCookie, ensureCsrfToken, verifyLogoutSuccess } from './auth';
  ...
  describe('Helper Functions', () => {
    describe('getCookie', () => {
      it('should extract cookie value correctly', () => {
        mockCookie.mockReturnValue('csrftoken=abc123; sessionid=def456; other=value');
    ...
    describe('ensureCsrfToken', () => {
      ...
      it('should handle network error during CSRF token request', async () => {
        ...
        const token = await ensureCsrfToken();
        expect(token).toBe(null);
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error getting CSRF token:', expect.any(Error));
        consoleErrorSpy.mockRestore();
      });
    });
    ...
    describe('verifyLogoutSuccess', () => {
      ...
      it('should handle network error during verification', async () => {
        ...
        const result = await verifyLogoutSuccess();
        expect(result).toBe(false);
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error verifying logout:', expect.any(Error));
        consoleErrorSpy.mockRestore();
      });
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle authentication state changes during loading', async () => {
      ...
      if (resolveFirstCall) {
        resolveFirstCall({
          ok: true,
          json: () => Promise.resolve({ authenticated: true, user: { id: 1, username: 'test' } }),
        } as Response);
      }
