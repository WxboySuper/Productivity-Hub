...
def test_create_project_success(client):
	...
	resp = client.post(PROJECTS_URL, json={"name": "Test Project", "description": "A test project."})
...
def test_update_project_success(client):
	...
	project_id = resp.get_json()["project_id"]
	resp = client.put(f"{PROJECTS_URL}/{project_id}", json={"name": "UpdatedName", "description": "UpdatedDesc"})
	assert resp.status_code == 200
	data = resp.get_json()
	assert data["success"] is True
	# Fetch and check
...
def test_update_project_not_owner(client):
	...
	user1 = register_and_login(client, username="owner1", email="owner1@weatherboysuper.com")
	resp = client.post(PROJECTS_URL, json={"name": "OwnerProj", "description": "Desc"})
	project_id = resp.get_json()["project_id"]
	# User2 tries to update
	client.post(LOGIN_URL, json={"username": "owner1", "password": "StrongPass1!"})  # logout user1
	user2 = register_and_login(client, username="owner2", email="owner2@weatherboysuper.com")
...
def test_delete_project_not_owner(client):
	# User1 creates project
	user1 = register_and_login(client, username="owner3", email="owner3@weatherboysuper.com")
	resp = client.post(PROJECTS_URL, json={"name": "OwnerProj2", "description": "Desc"})
	project_id = resp.get_json()["project_id"]
	# User2 tries to delete
	client.post(LOGIN_URL, json={"username": "owner3", "password": "StrongPass1!"})  # logout user1
	user2 = register_and_login(client, username="owner4", email="owner4@weatherboysuper.com")
	resp = client.delete(f"{PROJECTS_URL}/{project_id}")
	assert resp.status_code == 403
	assert "Not authorized" in resp.get_json().get("error", "")
