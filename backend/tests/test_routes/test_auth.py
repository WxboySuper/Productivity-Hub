...
from helpers.auth_helpers import generate_csrf_token, get_current_user, regenerate_session
...
def test_register_non_json_request(client):
	resp = client.post(REGISTER_URL, data="username=foo", content_type="application/x-www-form-urlencoded")
	assert resp.status_code == 400
...
def test_register_duplicate_username(client):
	...
	password = "StrongPass1!"
	client.post(REGISTER_URL, json={"username": username, "email": email1, "password": password})
	resp = client.post(REGISTER_URL, json={"username": username, "email": email2, "password": password})
	assert resp.status_code == 400
...
def test_register_duplicate_email(client):
	...
	password = "StrongPass1!"
	client.post(REGISTER_URL, json={"username": username1, "email": email, "password": password})
	resp = client.post(REGISTER_URL, json={"username": username2, "email": email, "password": password})
	assert resp.status_code == 400
...
def test_login_non_json_request(client):
	resp = client.post(LOGIN_URL, data="username=foo", content_type="application/x-www-form-urlencoded")
	assert resp.status_code == 400
	assert "Request must be JSON" in resp.get_json().get("error", "")
...
def test_login_wrong_password(client):
	...
	password = "StrongPass1!"
	client.post(REGISTER_URL, json={"username": username, "email": email, "password": password})
	resp = client.post(LOGIN_URL, json={"username": username, "password": "WrongPass2!"})
	assert resp.status_code == 401
	assert "Invalid credentials" in resp.get_json().get("error", "")

def test_login_nonexistent_user(client):
	resp = client.post(LOGIN_URL, json={"username": "nouser_" + secrets.token_hex(4), "password": "StrongPass1!"})
	assert resp.status_code == 401
	assert "Invalid credentials" in resp.get_json().get("error", "")
...
def test_profile_update_errors(client):
	...
	assert "username" in resp.get_json().get("error", {})
	# Duplicate username
	client.post(REGISTER_URL, json={"username": "takenuser", "email": f"taken_{unique}@weatherboysuper.com", "password": password})
	...
	assert "email" in resp.get_json().get("error", {})
	# Duplicate email
	client.post(REGISTER_URL, json={"username": f"othuser_{unique}", "email": f"dupemail_{unique}@weatherboysuper.com", "password": password})
	resp = client.put(PROFILE_URL, json={"email": f"dupemail_{unique}@weatherboysuper.com"})
	assert resp.status_code == 400
	assert "email" in resp.get_json().get("error", {})
...
def test_password_reset_request_non_json(client):
	resp = client.post("/api/password-reset/request", data="email=foo", content_type="application/x-www-form-urlencoded")
	assert resp.status_code == 400
	assert "Request must be JSON" in resp.get_json().get("error", "")
...
def test_password_reset_confirm_non_json(client):
	resp = client.post("/api/password-reset/confirm", data="token=foo", content_type="application/x-www-form-urlencoded")
	assert resp.status_code == 400
	assert "Request must be JSON" in resp.get_json().get("error", "")
