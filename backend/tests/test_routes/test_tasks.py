...
def test_get_tasks_subtask_due_start_fields(auth_client):
	...
	assert parent_task is not None
	subtask = next((st for st in parent_task["subtasks"] if st["id"] == subtask_id), None)
...
def test_update_task_description_and_completed_fields(auth_client):
	resp = auth_client.post(
		TASKS_URL, json={"title": "Update Desc", "priority": 1, "description": "Old desc", "completed": False}
	)
	assert resp.status_code == 201
	task_id = resp.get_json()["id"]
...
def test_update_task_remove_project_id(auth_client):
	...
	resp = auth_client.post(
		TASKS_URL, json={"title": "Task with Proj", "priority": 1, "project_id": project_id}
	)
	assert resp.status_code == 201
	task_id = resp.get_json()["id"]
	# Remove project_id
...
def test_update_task_project_id_and_due_date(auth_client):
	...
	resp = auth_client.post(
		TASKS_URL, json={"title": "Task to Move", "priority": 1, "project_id": project_id1, "due_date": "2025-10-01T10:00:00"}
	)
	assert resp.status_code == 201
	task_id = resp.get_json()["id"]
	# Update project_id and due_date
	resp = auth_client.put(
		f"{TASKS_URL}/{task_id}", json={"project_id": project_id2, "due_date": "2025-11-01T10:00:00"}
	)
	assert resp.status_code == 200
	data = resp.get_json()
...
def test_update_task_start_date_field(auth_client):
	resp = auth_client.post(
		TASKS_URL, json={"title": "Task Start Date", "priority": 1}
	)
	assert resp.status_code == 201
	task_id = resp.get_json()["id"]
	# Update start_date
