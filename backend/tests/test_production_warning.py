...
def test_warns_if_flask_env_production(monkeypatch):
    warnings_list = import_app_with_env_and_warning(monkeypatch, flask_env="production", environment=None)
...
def test_warns_if_environment_production(monkeypatch):
    warnings_list = import_app_with_env_and_warning(monkeypatch, flask_env=None, environment="production")
    assert any("production mode" in str(msg).lower() for msg, cat in warnings_list)
    assert any(cat == RuntimeWarning for msg, cat in warnings_list)

def test_no_warning_if_not_production(monkeypatch):
    warnings_list = import_app_with_env_and_warning(monkeypatch, flask_env="development", environment="development")
