...
def test_password_reset_request_invalid_email(mock_send_email, client, db):
    mock_send_email.return_value = True
    resp = client.post("/api/password-reset/request", json={"email": "nonexistent@example.com"})
    assert resp.status_code == 200
    data = resp.get_json()
    assert "message" in data
...
def test_password_reset_confirm_invalid_token(client, db, user):
    resp = client.post("/api/password-reset/confirm", json={"token": "invalid_token", "new_password": "NewStrongPassw0rd!"})
...
def test_password_reset_confirm_used_token(mock_send_email, client, db, user):
    ...
    token = resp.get_json()["token"]
    # Use the token once
    resp1 = client.post("/api/password-reset/confirm", json={"token": token, "new_password": "NewStrongPassw0rd!"})
...
def test_password_reset_confirm_weak_password(mock_send_email, client, db, user):
    ...
    data = resp2.get_json()
    assert "error" in data
    assert "characters" in data["error"].lower()  # Updated to match actual error message

def test_password_reset_token_expiration(client, db, user):
    # Create a token that is already expired
    expired_time = datetime.now(timezone.utc) - timedelta(minutes=1)
    prt = PasswordResetToken(user_id=user.id, token="expired_token", expires_at=expired_time)
    db.session.add(prt)
    db.session.commit()
    # Try to use the expired token
    resp = client.post("/api/password-reset/confirm", json={"token": "expired_token", "new_password": "NewStrongPassw0rd!"})
    assert resp.status_code == 400
    data = resp.get_json()
    assert "error" in data
